...
class LvAttnLoss(nn.Module):
    def __init__(self, frame_size, patch_size, num_frames):
        super().__init__()
    ...
    def forward(self, attn_pred, attn_label):

        with torch.no_grad():

            num_vids = attn_label.shape[1]
            batch_size = attn_label.shape[0]

            # Create the coarse version of the label
            # TODO: vectorize this
            label = torch.zeros(
                batch_size,
                num_vids,
            ...
            for i in range(batch_size):
                for j in range(num_vids):
                    for h in range(self.frame_size // self.patch_size):
                        for w in range(self.frame_size // self.patch_size):
                            label[i, j, h, w] = torch.any(
                                attn_label[
                                    i,
                                    j,
                                    h * self.patch_size : (h + 1) * self.patch_size,
                                    w * self.patch_size : (w + 1) * self.patch_size,
                                ]
                            )
        ...
        for i in range(batch_size):
            for j in range(num_vids):
                loss_to_add = self.l2_loss(
                    attn_pred[
                        i,
                        j * self.num_frames : (j + 1) * self.num_frames,
                        label[i, j] == False,
                    ],
                    torch.zeros_like(
                        attn_pred[
...
class FrameAttnLoss(nn.Module):
    def __init__(self):
        super().__init__()

        # self.binary_ct = nn.BCEWithLogitsLoss()
        self.l2_loss = nn.MSELoss()

    def forward(self, attn_pred, ed_frame_idx, ed_valid, es_frame_idx, es_valid):
        loss = 0

        if torch.any(ed_valid):
            ed_attn = attn_pred[ed_valid, ed_frame_idx[ed_valid]]
            loss += self.l2_loss(ed_attn, torch.ones_like(ed_attn)) / 2

        if torch.any(es_valid):
            es_attn = attn_pred[es_valid, es_frame_idx[es_valid]]
            loss += self.l2_loss(es_attn, torch.ones_like(es_attn)) / 2

        return loss


# THE FOLLOWING ARE FROM https://github.com/hitcszx/ALFs/blob/master/losses.py
eps = 1e-7
...
class AGCELoss(nn.Module):
    def __init__(self, num_classes=10, a=1, q=2, eps=eps, scale=1.0):
...
class NCEandAGCE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=10, a=3, q=1.5):
...
class NCEandAUE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=10, a=6, q=1.5):
...
class NCEandAEL(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=4.0, num_classes=10, a=2.5):
...
class NFLandAGCE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=10, a=3, q=2):
...
class NFLandAUE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=10, a=1.5, q=0.9):
...
class NFLandAEL(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=10, a=3):
...
class ANLandRCE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=10, a=2, p=0.7):
