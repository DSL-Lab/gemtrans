...
from .vit_configs import PRETRAINED_MODELS
...
def load_pretrained_weights(
    ...
    assert bool(model_name) ^ bool(
        weights_path
    ), "Expected exactly one of model_name or weights_path"
    ...
    if resize_positional_embedding:
        ...
        maybe_print(
            "Resized positional embeddings from {} to {}".format(
    ...
    if strict:
        ...
        maybe_print(
            "Unexpected keys when loading pretrained weights: {}".format(
                ret.unexpected_keys
            ),
            verbose,
        )
